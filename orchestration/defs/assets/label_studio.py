...
from orchestration.label_studio.converter import export_to_label_studio_tasks

@dg.asset(
  deps=["profiles"],
  config_schema={
    "output_dir": dg.Field(str, default_value="data/label_studio"),
  ...
  with database.get_connection() as conn:
    ads_with_entities = conn.execute("""
      SELECT 
        a.id,
        a.ad AS text,
        p.raw_llm_response->'entities' AS entities_json
      FROM ads a
      JOIN profiles p ON a.id = p.ad_id
      WHERE p.extraction_error IS NULL
        AND p.raw_llm_response IS NOT NULL
      LIMIT 500  -- Batch pour l'annotation humaine
    """).fetch_df()
...
@dg.asset(
  ...
  config_schema={
    ...
    "project_id": dg.Field(int, description="ID du projet Label Studio")
  }
)
def import_to_label_studio(context: dg.AssetExecutionContext) -> dg.MaterializeResult:
  ...
  tasks_path = Path("data/label_studio/tasks.json")
