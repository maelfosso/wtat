...
def records(
  ...
  """Transform sought, seeker JSON into tabular data"""
  
  with database.get_connection() as conn:
    raw_df = conn.execute("""
      SELECT ad_id, seeker, sought
      FROM raw_profiles
  ...
  context.log.info(f"✅ {len(flat_df)} tabular rows generated ({len(flat_df[flat_df['profile_type'] == 'SEEKER'])} SEEKER), {len(flat_df[flat_df['profile_type'] == 'SOUGHT'])}")
  ...
  return dg.MaterializeResult(
    metadata={
      ...
      "seeker_count": len(flat_df[flat_df["profile_type"] == "SEEKER"]),
      "sought_count": len(flat_df[flat_df["profile_type"] == "SOUGHT"]),
    }
  )
